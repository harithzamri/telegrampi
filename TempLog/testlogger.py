...
def databaseHelper(sqlCommand,sqloperation):
	try:
		...
		db = pyodbc.connect(
			...
			"UID = sa;"
			"PWD = gmtonline;"
			"Database = ebizos_db-201802060600;"
		)
		...
		if sqloperation == "Select":
			...
			try:
				...
				db.rollback()
				emailWarning("database inser failed", "")
			print ("error")
...
def checkWarningLog(sensor, sensortemp):
	...
	if data == None:
	       	sqlCommand = "INSERT INTO mailsendlog (mailsendtime, triggedsensor, triggedlimit ,lasttemperature) VALUES ('%s','%s','%s','%s')" % (currentTimeAsString,sensor,"0.0",sensortemp)
...
def main():
			...
			try:
				...
				if sensor1weeklyAverage != None and sensor1weeklyAverage != '':
					...
					if okToUpdate == True:
						...
						sqlCommand = "INSERT INTO mailsendlog (mailsendtime, triggedsensor, triggedlimit, lasttemperature) VALUES ('%s','%s','%s','%s')" % (currentTime,checkSensor,sensor1lowlimit,sensor1weeklyAverage)
						databaseHelper(sqlCommand,"Insert")
			except:
				...
				try:
					...
					if sensor2weeklyAverage != None and sensor2weeklyAverage != '':
						...
						if okToUpdate == True:
							...
							emailWarning(Message, msgType)
							sqlCommand = "INSERT INTO mailsendlog (mailsendtime, triggedsensor, triggedlimit ,lasttemperature) VALUES ('%s','%s','%s','%s')" % (currentTime,checkSensor,sensor2lowlimit,sensor2weeklyAverage)
							databaseHelper(sqlCommand,"Insert")
				except:
	...
	if sensor1error == 0:
		...
		if okToUpdate == True:
			...
			# Insert line to database to indicate when warning was sent
				currentTime = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
				sqlCommand = "INSERT INTO mailsendlog  (mailsendtime, triggedsensor, triggedlimit, lasttemperature) VALUES ('%s','%s','%s','%s')" % (currentTime,sensor1,sensor1lowlimit,sensor1temperature)
				databaseHelper(sqlCommand,"Insert")
			except:
	...
	if sensorsToRead != "1":
		...
		if sensor2error == 0:
			...
			if okToUpdate == True:
				...
				try:
					# Insert line to database to indicate when warning was sent
			       		sqlCommand = "INSERT INTO mailsendlog (mailsendtime, triggedsensor, triggedlimit ,lasttemperature) VALUES ('%s','%s','%s','%s')" % (currentTime,sensor2,sensor2lowlimit,sensor2temperature)
					databaseHelper(sqlCommand,"Insert")
				except:
	...
	try:
		if sensor1error == 0:
			sqlCommand = "INSERT INTO temperaturedata (dateandtime, sensor, temperature, humidity) VALUES ('%s','%s','%s','%s')" % (currentTime,sensor1,sensor1temperature,sensor1humidity)
		...
		if sensorsToRead != "1" and sensor2error == 0:
			sqlCommand = "INSERT INTO temperaturedata (dateandtime, sensor, temperature, humidity) VALUES ('%s','%s','%s','%s')" % (currentTime,sensor2,sensor2temperature,sensor2humidity)		
			# This row below sets temperature as fahrenheit instead of celsius. Comment above line and uncomment one below to take changes into use
